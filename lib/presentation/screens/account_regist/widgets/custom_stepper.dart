...
const double _kStepPadding = 10.0;
const double _kStepSpacing = 12.0;
const double _kStepFontSize = 19.0;
...
class _CustomStepperState extends State<CustomStepper>
  ...
  Color? _circleColor(int index) {
    ...
    return widget.steps[index].state != StepState.complete
        ? CupertinoDynamicColor.resolve(AppColors.whiteColor, context)
        : themeData.primaryColor;
  }
  ...
  Color _borderColor(int index) {
    ...
    if (widget.steps[index].state == StepState.complete ||
        widget.steps[index].state == StepState.editing) {
      return themeData.primaryColor;
    } else {
      return CupertinoDynamicColor.resolve(AppColors.borderColor, context);
    }
  }
    ...
    final List<Widget> children = <Widget>[
      for (int i = 0; i < widget.steps.length; i += 1) ...<Widget>[
        ...
        if (!_isLast(i))
          Expanded(
            child: Container(
              padding: const EdgeInsets.symmetric(horizontal: _kStepPadding),
              height: 10.0,
              width: 10.0,
              decoration: BoxDecoration(
                shape: BoxShape.circle,
                color: widget.steps[i].state != StepState.complete
